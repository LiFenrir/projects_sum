...
class Trainer:
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        # 可选：保存最佳指标对应的完整 checkpoint
        if checkpoint_names is None:
            checkpoint_names = []
        for ckpt_name in checkpoint_names:
            self._save_checkpoint(
                checkpoint,
                os.path.join(checkpoint_folder, f"checkpoint_{ckpt_name}.pt"),
            )

        # 按 save_freq 保存仅含模型权重的 checkpoint，key 与 sam3.pt 对齐
        save_list = getattr(self.checkpoint_conf, "save_list", [])
        should_save_model = (
            self.checkpoint_conf.save_freq > 0
            and (int(epoch) % self.checkpoint_conf.save_freq == 0)
        ) or int(epoch) in save_list
        if should_save_model:
            model_state = {f"detector.{k}": v for k, v in state_dict.items()}
            model_checkpoint = {"model": model_state}
            self._save_checkpoint(
                model_checkpoint,
                os.path.join(checkpoint_folder, f"model_{int(epoch)}.pt"),
            )
