...
def get_random_resize_scales_range(size, min_scale, max_scale, rounded=False):
    """生成 [size*min_scale, min(size*max_scale, size)] 候选尺寸列表，上限不超过 size"""
    stride = 128 if rounded else 32
    min_size = int(stride * math.ceil(size * min_scale / stride))
    max_size_val = min(int(size * max_scale), size)
    if max_size_val < min_size:
        max_size_val = min_size
    scales = list(range(min_size, max_size_val + 1, stride))
    return scales
