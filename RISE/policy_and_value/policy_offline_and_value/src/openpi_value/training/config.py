...
import openpi_value.policies.lerobot_policy as lerobot_policy
...
class LeRobotDataConfig(DataConfigFactory):
    """Data config for generic LeRobot-compatible robots.

    Unlike `LerobotCustomAgilexDataConfig`, this config does not hard-code
    camera names. Instead, ``camera_map`` maps robodeploy/LeRobot camera
    names to OpenPI model image slots. Missing cameras are zero-filled and
    masked out (or broadcast from the base camera when ``broadcast_base`` is
    True).
    ...
    # Mapping from robodeploy/LeRobot camera names to OpenPI model image slots.
    camera_map: dict[str, str] = dataclasses.field(default_factory=dict)

    # If True, broadcast the base camera to wrist slots when they are missing.
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        data_transforms = _transforms.Group(
            inputs=[
                lerobot_policy.LeRobotInputs(
                    action_dim=model_config.action_dim,
                    camera_map=self.camera_map,
                    broadcast_base=self.broadcast_base,
...
_CONFIGS = [
    ...
    # * policy: generic LeRobot pi05 style
    TrainConfig(

        name="LeRobot_pi05_finetune",
        ...
        data = LeRobotDataConfig(
            repo_id = "/home/kemove/INNOV/datasets/innov_arm/innov_0730_0731_3cam",

            assets=AssetsConfig(
                assets_dir="data/norms",
                asset_id="innov_0730_0731_3cam",
            ),

            default_prompt="Put the water flosser into the box and close the lid",
            use_delta_joint_actions=False,

            robot_type="innov_arm",
        ...
        pytorch_weight_path="/home/kemove/INNOV/models/openpi/pi05_base_pytorch",
                ...
                _transforms.RepackTransform(
                    {
                        "images": {
                            ...
                            "fut_1_top_head": "fut_1_observation.images.top_head",
                            "fut_1_hand_left": "fut_1_observation.images.hand_left",
                            "fut_1_hand_right": "fut_1_observation.images.hand_right",
                        },
                        "state": "observation.state",
                        "actions": "action",
                        "prompt": "prompt",
