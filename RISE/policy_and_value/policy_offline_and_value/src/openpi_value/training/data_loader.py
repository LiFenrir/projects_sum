...
def create_torch_dataset_naive(
    ...
    if len(repo_id) > 1:
        ...
        dataset_meta = lerobot_dataset.LeRobotDatasetMetadata(repo_id[0], root=repo_id[0])  # * Just use the first repo to get fps
        dataset = lerobot_dataset.MultiLeRobotDataset(
            repo_id,
            root={rp: rp for rp in repo_id},
            delta_timestamps={
        ...
        # * Filter out episodes with missing data/video files to avoid HF download fallback.
        root_path = pathlib.Path(repo_id)
        valid_episodes = []
        for ep_idx in range(dataset_meta.total_episodes):
            chunk = ep_idx // dataset_meta.chunks_size
            data_file = root_path / dataset_meta.get_data_file_path(ep_idx)
            if not data_file.is_file():
                logging.warning("Skipping episode %d due to missing data file: %s", ep_idx, data_file)
                continue
            missing_video = False
            for vid_key in dataset_meta.video_keys:
                video_file = root_path / dataset_meta.get_video_file_path(ep_idx, vid_key)
                if not video_file.is_file():
                    logging.warning(
                        "Skipping episode %d due to missing video file: %s", ep_idx, video_file
                    )
                    missing_video = True
                    break
            if missing_video:
        ...
        if len(valid_episodes) == 0:
            raise ValueError(f"No valid episodes found in {repo_id}. Check data/video files.")

        logging.info(
            "Using %d / %d episodes from %s (skipped %d with missing files)",
            len(valid_episodes),
            dataset_meta.total_episodes,
            repo_id,
            dataset_meta.total_episodes - len(valid_episodes),
        )
