...
import openpi_value.models.pi0_config
...
def load_checkpoint(model, optimizer, checkpoint_dir, device):
    ...
    try:
        ...
        if safetensors_path.exists():
            ...
            logging.info(f"Loaded model state from pt format: {pt_path}")
        else:
        ...
        if optimizer_path.exists():
            ...
            raise FileNotFoundError(f"No optimizer checkpoint found in {ckpt_dir}")
        ...
        if (ckpt_dir / "metadata.pt").exists():
...
def train_loop(config: _config.TrainConfig):
    ...
    if isinstance(config.model, openpi_value.models.pi0_config.Pi0Config) or \
        isinstance(config.model, openpi_value.models.pi0_config.Pi0Config_Custom):

        model_cfg = config.model
        object.__setattr__(model_cfg, "dtype", config.pytorch_training_precision)
    else:
        model_cfg = openpi_value.models.pi0_config.Pi0Config(
    ...
    model = openpi_value.models_pytorch.pi0_pytorch.PI0Pytorch(model_cfg).to(device)
    ...
    if config.pytorch_weight_path is not None:
        ...
        pt_path = os.path.join(config.pytorch_weight_path, "model.pt")
        ...
        if os.path.exists(model_path):
            ...
            raise FileNotFoundError(
                f"No model checkpoint found in {config.pytorch_weight_path}; "
