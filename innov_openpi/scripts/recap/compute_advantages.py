...
from recap.data.utils import (
    decode_image_struct_batch,
    load_return_stats_from_dataset,
    load_returns_sidecar,
)
from recap.models.value_critic.modeling_critic import ValueCriticModel

logger = logging.getLogger(__name__)


def setup_distributed(cfg: dict) -> tuple[int, int, str]:
...
def _parse_value_model_kwargs(cfg: dict) -> dict:
    """Extract ValueCriticModel.from_checkpoint kwargs from config dict."""
    ...
    camera_map = data_cfg.get("camera_map", None)
    if camera_map is None:
        raise ValueError(
            "data.camera_map is required. Set it in the config, e.g.:\n"
            "  data:\n"
            "    camera_map:\n"
            "      front: base_0_rgb\n"
    ...
    return {
        ...
        "asset_id": data_cfg.get("asset_id", None),
...
def _resolve_prompt(sample: dict[str, Any], tasks: dict[int, str]) -> str:
    """Resolve task prompt from LeRobot sample (task text or task_index lookup)."""
    if "task" in sample:
        val = sample["task"]
    ...
    if "task_index" in sample and tasks:
        idx = int(
            sample["task_index"].item()
            if hasattr(sample["task_index"], "item")
            else sample["task_index"]
        )
        if idx not in tasks:
            raise ValueError(
                f"task_index {idx} not found in tasks dict. "
...
class ValueInferenceDataset(torch.utils.data.Dataset):
    ...
    Passes raw LeRobot samples directly through _input_transform
    (RepackTransform + LeRobotInputs, matching LeRobotDataConfig pattern).
    """
    ...
    def __getitem__(self, idx):
        ...
        # Inject prompt (task_index → task text) for LeRobotInputs
        obs = dict(sample)
        if "prompt" not in obs:
            obs["prompt"] = _resolve_prompt(sample, self.tasks)
...
def main() -> None:
    ...
    """
    # Parse CLI args (must be done before setup_distributed so cfg is ready)
    parser = argparse.ArgumentParser(
        description="Compute advantages for CFG-RL training"
    )
    parser.add_argument("--config", type=str, default=None,
                        help="YAML config file path")
    parser.add_argument("--value_checkpoint", type=str, required=True,
                        help="Path to trained value model checkpoint")
    parser.add_argument("--datasets", type=str, nargs="+", default=[],
                        help="Dataset paths")
    parser.add_argument("--robot_type", type=str, default="libero")
    parser.add_argument("--model_type", type=str, default="pi05")
    parser.add_argument("--advantage_lookahead_step", type=int, default=10)
    parser.add_argument("--gamma", type=float, default=1.0)
    parser.add_argument("--positive_quantile", type=float, default=0.3)
    parser.add_argument("--batch_size", type=int, default=2048)
    parser.add_argument("--tag", type=str, default=None)
    parser.add_argument("--max_samples", type=int, default=None)
    parser.add_argument("--return_min", type=float, default=None)
    parser.add_argument("--return_max", type=float, default=None)
    ...
    # Ensure advantage section exists
    cfg.setdefault("advantage", {})
    cfg["advantage"]["value_checkpoint"] = cli_args.value_checkpoint
    cfg.setdefault("data", {})
    if cli_args.datasets:
        cfg["data"]["dataset_paths"] = cli_args.datasets
        cfg["data"]["train_data_paths"] = [
            {"dataset_path": p, "robot_type": cli_args.robot_type, "type": "rollout", "weight": 1.0}
            for p in cli_args.datasets
        ]
    cfg["data"].setdefault("robot_type", cli_args.robot_type)
    cfg["data"].setdefault("model_type", cli_args.model_type)
    cfg["data"].setdefault("gamma", cli_args.gamma)
    cfg["data"].setdefault("advantage_lookahead_step", cli_args.advantage_lookahead_step)
    cfg["advantage"].setdefault("positive_quantile", cli_args.positive_quantile)
    cfg["advantage"].setdefault("batch_size", cli_args.batch_size)
    if cli_args.tag:
        cfg["advantage"]["tag"] = cli_args.tag
    if cli_args.max_samples:
        cfg["advantage"]["max_samples"] = cli_args.max_samples
    if cli_args.return_min is not None:
        cfg["data"]["return_min"] = cli_args.return_min
    if cli_args.return_max is not None:
        cfg["data"]["return_max"] = cli_args.return_max

    # Fallback config for defaults
    cfg.setdefault("distributed", {})
    cfg["distributed"].setdefault("enabled", True)
    cfg["distributed"].setdefault("backend", "nccl")
    cfg["distributed"].setdefault("timeout", 3600)
    cfg["advantage"].setdefault("model", {})
    cfg["advantage"].setdefault("discount_next_value", True)
    cfg["advantage"].setdefault("flush_interval", 128)
    cfg["advantage"].setdefault("num_dataloader_workers_per_gpu", 8)
    cfg["advantage"].setdefault("prefetch_factor", 2)
    cfg["advantage"].setdefault("num_dataloader_workers", 8)
    cfg["advantage"].setdefault("returns_tag", cfg["advantage"].get("tag"))
