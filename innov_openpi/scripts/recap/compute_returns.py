...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Compute returns for LeRobot datasets"
    )
    parser.add_argument("--config", type=str, default=None,
                        help="YAML config file path")
    parser.add_argument("--dataset_path", type=str, default=None,
                        help="Path to a single dataset")
    parser.add_argument("--datasets", type=str, nargs="+", default=None,
                        help="List of dataset paths")
    parser.add_argument("--data_root", type=str, default=None,
                        help="Root data directory for relative paths")
    parser.add_argument("--dataset_type", type=str, default="sft",
                        choices=["sft", "rollout"],
                        help="Dataset type")
    parser.add_argument("--gamma", type=float, default=1.0,
                        help="Discount factor")
    parser.add_argument("--failure_reward", type=float, default=-300.0,
                        help="Reward for failed episode's last step")
    parser.add_argument("--num_workers", type=int, default=8,
                        help="Parallel worker threads")
    parser.add_argument("--tag", type=str, default=None,
                        help="Tag for output filename")
    parser.add_argument("--output_path", type=str, default=None,
                        help="Output path (null=in-place)")
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO)
    logger.info("Starting return computation...")

    # Load config from YAML if provided, CLI args override
    data_root = args.data_root
    default_type = args.dataset_type
    default_gamma = args.gamma
    default_failure_reward = args.failure_reward
    ...
    if args.config:
        ...
        data_root = data_root or data_cfg.get("data_root")
        default_type = data_cfg.get("dataset_type", default_type)
        default_gamma = data_cfg.get("gamma", default_gamma)
        default_failure_reward = data_cfg.get("failure_reward", default_failure_reward)
        num_workers = data_cfg.get("num_workers", num_workers)
        tag = data_cfg.get("tag", tag)
    ...
    if datasets_list and len(datasets_list) > 0:
        for entry in datasets_list:
            ...
            output_path = entry.get("output_path", default_output_path)
            ...
            datasets_to_process.append({
                ...
                "failure_reward": entry.get("failure_reward", default_failure_reward),
            })
        ...
        raise ValueError(
            "No datasets specified. Use --config, --dataset_path, or --datasets."
        )
