...
class ValueCriticModel(nn.Module):
    ...
    def from_checkpoint(
        ...
        *,
        ...
        from .checkpoint_utils import has_tokenizer_files, load_norm_stats
        ...
        # Ensure uniform bf16 dtype for inference — _apply_precision's
        # mixed-precision path (fp32 patch embeddings + bf16 norms) is
        # only safe under autocast during training; inference needs
        # consistent dtype across the vision tower.
        model = model.to(dtype=torch.bfloat16)
        model.eval()

        # Load norm stats (keep existing path: checkpoint_dir/assets/{asset_id}/norm_stats.json)
        if norm_stats is None and asset_id is not None:
            try:
                norm_stats = load_norm_stats(checkpoint_dir / "assets", asset_id)
                if norm_stats is not None:
                    logger.info(f"Loaded norm stats: assets/{asset_id}/norm_stats.json")
            except FileNotFoundError:
                logger.warning(
                    f"Norm stats not found at {checkpoint_dir}/assets/{asset_id}, "
                    "proceeding without normalization"
                )

        # Strip "return" key and guard against empty dict after filtering
        if norm_stats and "return" in norm_stats:
            norm_stats = {k: v for k, v in norm_stats.items() if k != "return"}
        if norm_stats is not None and not norm_stats:
            logger.warning("norm_stats is empty after filtering, disabling normalization")
            norm_stats = None

        use_quantile_norm = model_type.lower() != "pi0"

        if not camera_map:
            raise ValueError("camera_map is required and must be non-empty (see LeRobotDataConfig).")

        from openpi.policies.lerobot_policy import LeRobotInputs
        from openpi.transforms import Normalize, PadStatesAndActions, RepackTransform, compose

        action_dim = getattr(model.config, "action_dim", 32)

        # Build inference transform chain following DataConfig pattern:
        # repack → data_transforms → Normalize → PadStatesAndActions.
        # Uses LeRobotInputs directly (no JAX/Flax dependency) while
        # aligning Normalize injection point with RLinf.
        images_repack = {cam: f"observation.images.{cam}" for cam in camera_map}
        _transforms: list = [
            RepackTransform(
                {"images": images_repack, "state": "observation.state", "actions": "action"}
            ),
            LeRobotInputs(camera_map=camera_map, default_prompt=default_prompt),
        ]
        if norm_stats is not None:
            _transforms.append(Normalize(norm_stats, use_quantiles=use_quantile_norm))
        _transforms.append(PadStatesAndActions(action_dim))

        model._input_transform = compose(_transforms)
        # Register device as a persistent buffer so model.to() keeps it in sync
        model.register_buffer("_device_tensor", torch.empty(0, device=device), persistent=True)
