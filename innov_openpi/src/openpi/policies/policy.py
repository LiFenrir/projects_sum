...
class Policy(BasePolicy):
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            action_chunk: Number of action steps to return per inference. If None, returns the
                          model's full action_horizon. Must be <= model.action_horizon.
        """
        self._model = model
    ...
    @override
    def infer(self, obs: dict, *, noise: np.ndarray | None = None, **rtc_kwargs) -> dict:  # type: ignore[misc]
        ...
        # Truncate action chunk if requested
        if self._action_chunk is not None:
            outputs["actions"] = outputs["actions"][: self._action_chunk]

        outputs["policy_timing"] = {
            "infer_ms": model_time * 1000,
        }

        logging.info("infer | time=%5.0fms", model_time * 1000)
        return outputs
