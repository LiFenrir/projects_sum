...
class Sam3PredictorAdapter:
    ...
    def predict_texts(self, text_prompts, confidence_threshold=None):
        """对同一张图依次推理多个文本提示词,合并所有实例后统一返回。

        输入: text_prompts 提示词列表; 输出 (masks (N,H,W) bool,
        scores (N,), boxes (N,4) XYXY, prompt_ids (N,)),按 score 降序,
        prompt_ids[i] 标记实例 i 来自哪个提示词。不同提示词可能重复检出
        同一实例,调用方按需取并集或去重。
        """
        all_masks, all_scores, all_boxes, all_pids = [], [], [], []
        for pid, prompt in enumerate(text_prompts):
            masks, scores, boxes = self.predict_text(
                prompt, confidence_threshold=confidence_threshold)
            all_masks.append(masks)
            all_scores.append(scores)
            all_boxes.append(boxes)
            all_pids.append(np.full(len(scores), pid, dtype=np.int64))

        if sum(len(s) for s in all_scores) == 0:
            h, w = self._state["original_height"], self._state["original_width"]
            return (np.zeros((0, h, w), dtype=bool), np.zeros(0),
        ...
        order = np.argsort(-scores)
        return masks[order], scores[order], boxes[order], pids[order]
