...
# because the rendered robot arm covers that region anyway. 默认用微调模型的
# 四个类别一起检出,保证手臂区域完整覆盖。
DEFAULT_TARGETS = ["hair", "hand", "forearm", "whole hand and forearm"]
TARGETS_WITH_OBJECT = DEFAULT_TARGETS + ["the object being held"]
...
def _seed_masks(tracker, frame_dir, targets, confidence, segmentor,
                offload) -> Optional[List[np.ndarray]]:
    """Segment `targets` on the first frame that contains them, then propagate.

    多个提示词依次推理后取并集作为初始 mask。Egocentric clips often start
    with the hands out of view, so a failure on frame 0 is not a failure of
    the episode.
    """
...
def process_episode(models, ep: Episode, args) -> dict:
    ...
    with _as_frame_folder(str(ep.source)) as frame_dir:
        ...
        raw_masks = _seed_masks(
            models["tracker"], frame_dir, args.targets, args.text_confidence,
            models["segmentor"], args.offload)

        if raw_masks is None:
            entry.update(status="no_target",
                         note=f"{args.targets!r} not found in any probed frame")
            return entry
...
def _inpaint_chunked(painter, frames, masks, args, inpaint_fn=None):
    """Run the video inpainter, optionally in overlapping chunks to bound memory."""
    if inpaint_fn is None:
        def _propainter_fn(fr, mk):
            return inpaint_video_with_builded_propainter(
                painter, fr, mk, device=args.device, fp16=args.fp16,
                subvideo_length=args.subvideo_length)
        inpaint_fn = _propainter_fn
    n = len(frames)
    if args.chunk_size <= 0 or n <= args.chunk_size:
        return inpaint_fn(frames, masks)
    ...
    while start < n:
        ...
        chunk = inpaint_fn(frames[lo:end], masks[lo:end])
...
def _load_detector_ckpt(detector, ckpt_p: str):
    """把微调检测器权重(detector.* 键)覆盖加载到视频模型的检测器上。

    tracker 部分保持基础权重不变;微调附带而模型中不存在的键
    (如训练用 semantic_seg_head)直接忽略。
    """
    ckpt = torch.load(ckpt_p, map_location="cpu", weights_only=True)
    if "model" in ckpt and isinstance(ckpt["model"], dict):
        ckpt = ckpt["model"]
    det_sd = {k[len("detector."):]: v for k, v in ckpt.items()
              if k.startswith("detector.")}
    if not det_sd:
        raise ValueError(f"No 'detector.*' keys found in {ckpt_p}")
    model_keys = set(detector.state_dict())
    dropped = sorted(k for k in det_sd if k not in model_keys)
    det_sd = {k: v for k, v in det_sd.items() if k in model_keys}
    missing, _ = detector.load_state_dict(det_sd, strict=False)
    print(f"Loaded finetuned detector from {ckpt_p}: "
          f"{len(det_sd)} keys, {len(missing)} missing, "
...
def build_models(args):
    ...
    if args.detector_ckpt:
        _load_detector_ckpt(tracker.text_segmentor.model, args.detector_ckpt)
...
def setup_args(parser):
    ...
    parser.add_argument(
        ...
        help=f"What gets erased, as one or more text prompts. Each prompt is "
             f"run separately and the masks are merged. Defaults to "
             f"{DEFAULT_TARGETS!r} ({TARGETS_WITH_OBJECT!r} in remove_object "
             f"mode).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--detector_ckpt", type=str, default=None,
        help="可选的微调检测器权重(仅含 detector.* 键),加载后覆盖 "
             "--sam_ckpt 中的检测器部分,tracker 保持基础权重不变。",
    )
    parser.add_argument(
...
def main():
    ...
    print(f"Erasing: {args.targets!r}")
    ...
    for i, ep in enumerate(episodes, 1):
        ...
        (out_dir / "manifest.json").write_text(
            json.dumps({"targets": args.targets, "episodes": manifest}, indent=2))
