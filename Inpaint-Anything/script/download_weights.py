...
def download_diffueraser():
    """DiffuEraser video inpainting: its weights plus SD1.5/VAE/PCM parts.

    Lands in diffueraser/weights/ following the upstream layout. ProPainter
    (the prior model) is shared with the main pipeline: download_propainter.
    """
    weight_root = ROOT / "diffueraser" / "weights"
    # (HF repo, ModelScope mirror, subdir, allow_patterns)
    repos = [
        ("lixiaowen/diffuEraser", "xingzi/diffuEraser", "diffuEraser", None),
        ("stable-diffusion-v1-5/stable-diffusion-v1-5",
         "AI-ModelScope/stable-diffusion-v1-5", "stable-diffusion-v1-5",
         ["feature_extractor/*", "model_index.json", "safety_checker/*",
          "scheduler/*", "text_encoder/*", "tokenizer/*"]),
        ("wangfuyun/PCM_Weights", "AI-ModelScope/PCM_Weights",
         "PCM_Weights", ["sd15/*"]),
        ("stabilityai/sd-vae-ft-mse", "AI-ModelScope/sd-vae-ft-mse",
         "sd-vae-ft-mse", None),
    ]
    ok = True
    for hf_repo, ms_repo, subdir, patterns in repos:
        dst = weight_root / subdir
        if dst.exists() and any(dst.iterdir()):
            print(f"  {subdir} already present, skipping ({dst})")
            continue
        try:
            from huggingface_hub import snapshot_download
            snapshot_download(hf_repo, local_dir=str(dst),
        ...
        try:
            ...
            ms_snapshot_download(ms_repo, local_dir=str(dst),
                                 allow_patterns=patterns)
            print(f"  saved {dst} (from ModelScope)")
        except Exception as e:
            print(f"  ModelScope failed too: {type(e).__name__}: {e}")
...
TARGETS = {
    ...
    "flux": download_flux,
    "diffueraser": download_diffueraser,
    "sttn": download_sttn,
